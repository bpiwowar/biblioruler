# coding: utf-8
from sqlalchemy import Boolean, Column, Date, DateTime, ForeignKey, Integer, Numeric, Table, Text, UniqueConstraint
...
class Annotation(Base):
    ...
    itemID = Column(ForeignKey('items.itemID', ondelete='CASCADE'), nullable=False, index=True)
    parent = Column(Text)
...
class BaseFieldMapping(Base):
    ...
    field = relationship('Field', primaryjoin='BaseFieldMapping.baseFieldID == Field.fieldID', backref='field_base_field_mappings')
    field1 = relationship('Field', primaryjoin='BaseFieldMapping.fieldID == Field.fieldID', backref='field_base_field_mappings_0')
    itemType = relationship('ItemType', primaryjoin='BaseFieldMapping.itemTypeID == ItemType.itemTypeID', backref='base_field_mappings')
...
class CollectionItem(Base):
    ...
    collectionID = Column(ForeignKey('collections.collectionID', ondelete='CASCADE'), primary_key=True, nullable=False)
    itemID = Column(ForeignKey('items.itemID', ondelete='CASCADE'), primary_key=True, nullable=False, index=True)
    orderIndex = Column(Integer, nullable=False, server_default=FetchedValue())

    collection = relationship('Collection', primaryjoin='CollectionItem.collectionID == Collection.collectionID', backref='collection_items')
    item = relationship('Item', primaryjoin='CollectionItem.itemID == Item.itemID', backref='collection_items')
...
class CollectionRelation(Base):
    ...
    collectionID = Column(ForeignKey('collections.collectionID', ondelete='CASCADE'), primary_key=True, nullable=False)
    predicateID = Column(ForeignKey('relationPredicates.predicateID', ondelete='CASCADE'), primary_key=True, nullable=False, index=True)
    object = Column(Text, primary_key=True, nullable=False, index=True)

    collection = relationship('Collection', primaryjoin='CollectionRelation.collectionID == Collection.collectionID', backref='collection_relations')
    relationPredicate = relationship('RelationPredicate', primaryjoin='CollectionRelation.predicateID == RelationPredicate.predicateID', backref='collection_relations')
...
class Collection(Base):
    ...
    parentCollectionID = Column(ForeignKey('collections.collectionID', ondelete='CASCADE'), server_default=FetchedValue())
    clientDateModified = Column(DateTime, nullable=False, server_default=FetchedValue())
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), nullable=False)
    ...
    library = relationship('Library', primaryjoin='Collection.libraryID == Library.libraryID', backref='collections')
    parent = relationship('Collection', remote_side=[collectionID], primaryjoin='Collection.parentCollectionID == Collection.collectionID', backref='collections')
...
class Creator(Base):
    ...
    __table_args__ = (
        UniqueConstraint('lastName', 'firstName', 'fieldMode'),
    )
...
class CustomBaseFieldMapping(Base):
    ...
    field = relationship('Field', primaryjoin='CustomBaseFieldMapping.baseFieldID == Field.fieldID', backref='custom_base_field_mappings')
    customField = relationship('CustomField', primaryjoin='CustomBaseFieldMapping.customFieldID == CustomField.customFieldID', backref='custom_base_field_mappings')
    customItemType = relationship('CustomItemType', primaryjoin='CustomBaseFieldMapping.customItemTypeID == CustomItemType.customItemTypeID', backref='custom_base_field_mappings')
...
class CustomItemTypeField(Base):
    ...
    customField = relationship('CustomField', primaryjoin='CustomItemTypeField.customFieldID == CustomField.customFieldID', backref='custom_item_type_fields')
    customItemType = relationship('CustomItemType', primaryjoin='CustomItemTypeField.customItemTypeID == CustomItemType.customItemTypeID', backref='custom_item_type_fields')
    field = relationship('Field', primaryjoin='CustomItemTypeField.fieldID == Field.fieldID', backref='custom_item_type_fields')
...
class Field(Base):
    ...
    fieldFormat = relationship('FieldFormat', primaryjoin='Field.fieldFormatID == FieldFormat.fieldFormatID', backref='fields')
...
class FileTypeMimeType(Base):
    ...
    fileType = relationship('FileType', primaryjoin='FileTypeMimeType.fileTypeID == FileType.fileTypeID', backref='file_type_mime_types')
...
t_fulltextItemWords = Table(
    ...
    Column('itemID', ForeignKey('items.itemID', ondelete='CASCADE'), primary_key=True, nullable=False, index=True)
)
...
class Group(Base):
    ...
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), nullable=False, unique=True)
    name = Column(Text, nullable=False)
    description = Column(Text, nullable=False)
    version = Column(Integer, nullable=False)

    library = relationship('Library', uselist=False, primaryjoin='Group.libraryID == Library.libraryID', backref='groups')
...
class Highlight(Base):
    ...
    itemID = Column(ForeignKey('item.itemID', ondelete='CASCADE'), nullable=False, index=True)
...
class ItemCreator(Base):
    ...
    itemID = Column(ForeignKey('items.itemID', ondelete='CASCADE'), primary_key=True, nullable=False)
    creatorID = Column(ForeignKey('creators.creatorID', ondelete='CASCADE'), primary_key=True, nullable=False)
    creatorTypeID = Column(ForeignKey('creatorTypes.creatorTypeID'), primary_key=True, nullable=False, index=True, server_default=FetchedValue())
    orderIndex = Column(Integer, primary_key=True, nullable=False, server_default=FetchedValue())

    creator = relationship('Creator', primaryjoin='ItemCreator.creatorID == Creator.creatorID', backref='item_creators')
    creatorType = relationship('CreatorType', primaryjoin='ItemCreator.creatorTypeID == CreatorType.creatorTypeID', backref='item_creators')
    item = relationship('Item', primaryjoin='ItemCreator.itemID == Item.itemID', backref='item_creators')
...
class ItemData(Base):
    ...
    itemID = Column(ForeignKey('items.itemID', ondelete='CASCADE'), primary_key=True, nullable=False)
    fieldID = Column(ForeignKey('fieldsCombined.fieldID'), primary_key=True, nullable=False, index=True)
    valueID = Column(ForeignKey('itemDataValues.valueID'))

    field = relationship('FieldsCombined', primaryjoin='ItemData.fieldID == FieldsCombined.fieldID', backref='item_data')
    item = relationship('Item', primaryjoin='ItemData.itemID == Item.itemID', backref='data')
    data = relationship('ItemDataValue', primaryjoin='ItemData.valueID == ItemDataValue.valueID', backref='item_data')
...
class ItemRelation(Base):
    ...
    item = relationship('Item', primaryjoin='ItemRelation.itemID == Item.itemID', backref='item_relations')
    relationPredicate = relationship('RelationPredicate', primaryjoin='ItemRelation.predicateID == RelationPredicate.predicateID', backref='item_relations')
...
class ItemTag(Base):
    ...
    itemID = Column(ForeignKey('items.itemID', ondelete='CASCADE'), primary_key=True, nullable=False)
    tagID = Column(ForeignKey('tags.tagID', ondelete='CASCADE'), primary_key=True, nullable=False, index=True)
    type = Column(Integer, nullable=False)

    item = relationship('Item', primaryjoin='ItemTag.itemID == Item.itemID', backref='item_tags')
    tag = relationship('Tag', primaryjoin='ItemTag.tagID == Tag.tagID', backref='item_tags')
...
class ItemTypeCreatorType(Base):
    ...
    creatorType = relationship('CreatorType', primaryjoin='ItemTypeCreatorType.creatorTypeID == CreatorType.creatorTypeID', backref='item_type_creator_types')
    itemType = relationship('ItemType', primaryjoin='ItemTypeCreatorType.itemTypeID == ItemType.itemTypeID', backref='item_type_creator_types')
...
class ItemTypeField(Base):
    ...
    field = relationship('Field', primaryjoin='ItemTypeField.fieldID == Field.fieldID', backref='item_type_fields')
    itemType = relationship('ItemType', primaryjoin='ItemTypeField.itemTypeID == ItemType.itemTypeID', backref='item_type_fields')
...
class Item(Base):
    ...
    library = relationship('Library', primaryjoin='Item.libraryID == Library.libraryID', backref='items')
...
class DeletedItem(Item):
    ...
    dateDeleted = Column(NullType, nullable=False, index=True, server_default=FetchedValue())
...
class GroupItem(Item):
    ...
    user = relationship('User', primaryjoin='GroupItem.createdByUserID == User.userID', backref='user_group_items')
    user1 = relationship('User', primaryjoin='GroupItem.lastModifiedByUserID == User.userID', backref='user_group_items_0')
...
class Feed(Library):
    ...
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), primary_key=True)
    name = Column(Text, nullable=False)
...
class ProxyHost(Base):
    ...
    proxy = relationship('Proxy', primaryjoin='ProxyHost.proxyID == Proxy.proxyID', backref='proxy_hosts')
...
class SavedSearchCondition(Base):
    ...
    savedSearchID = Column(ForeignKey('savedSearches.savedSearchID', ondelete='CASCADE'), primary_key=True, nullable=False)
    ...
    savedSearch = relationship('SavedSearch', primaryjoin='SavedSearchCondition.savedSearchID == SavedSearch.savedSearchID', backref='saved_search_conditions')
...
class SavedSearch(Base):
    ...
    library = relationship('Library', primaryjoin='SavedSearch.libraryID == Library.libraryID', backref='saved_searches')
...
class StorageDeleteLog(Base):
    ...
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), primary_key=True, nullable=False)
    key = Column(Text, primary_key=True, nullable=False)
    dateDeleted = Column(Text, nullable=False, server_default=FetchedValue())

    library = relationship('Library', primaryjoin='StorageDeleteLog.libraryID == Library.libraryID', backref='storage_delete_logs')
...
class SyncCache(Base):
    ...
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), primary_key=True, nullable=False)
    key = Column(Text, primary_key=True, nullable=False)
    syncObjectTypeID = Column(ForeignKey('syncObjectTypes.syncObjectTypeID'), primary_key=True, nullable=False)
    version = Column(Integer, primary_key=True, nullable=False)
    data = Column(Text)

    library = relationship('Library', primaryjoin='SyncCache.libraryID == Library.libraryID', backref='sync_caches')
    syncObjectType = relationship('SyncObjectType', primaryjoin='SyncCache.syncObjectTypeID == SyncObjectType.syncObjectTypeID', backref='sync_caches')
...
t_syncDeleteLog = Table(
    ...
    Column('libraryID', ForeignKey('libraries.libraryID', ondelete='CASCADE'), nullable=False),
...
class SyncQueue(Base):
    ...
    libraryID = Column(ForeignKey('libraries.libraryID', ondelete='CASCADE'), primary_key=True, nullable=False)
    key = Column(Text, primary_key=True, nullable=False)
    syncObjectTypeID = Column(ForeignKey('syncObjectTypes.syncObjectTypeID', ondelete='CASCADE'), primary_key=True, nullable=False)
    lastCheck = Column(DateTime)
    tries = Column(Integer)

    library = relationship('Library', primaryjoin='SyncQueue.libraryID == Library.libraryID', backref='sync_queues')
    syncObjectType = relationship('SyncObjectType', primaryjoin='SyncQueue.syncObjectTypeID == SyncObjectType.syncObjectTypeID', backref='sync_queues')
...
class SyncedSetting(Base):
    ...
    library = relationship('Library', primaryjoin='SyncedSetting.libraryID == Library.libraryID', backref='synced_settings')
...
class TransactionLog(Base):
    ...
    transaction = relationship('Transaction', primaryjoin='TransactionLog.transactionID == Transaction.transactionID', backref='transaction_logs')
