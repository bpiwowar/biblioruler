...
def command_export(args, remaining_args):
    source, remaining_args = configure_manager("source-", args.source, remaining_args)
    exporter, remaining_args = configure_exporter(
        "exporter-", args.exporter, remaining_args
    )
...
parser.add_argument(
    "--config",
    action="store_true",
    default=op.expanduser("~/.biblioruler"),
...
parser_export.add_argument(
    "path",
    help="The output path (without extension) - this can be either a folder or a file, depending on the exporter",
)
